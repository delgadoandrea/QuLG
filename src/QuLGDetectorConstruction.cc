...
G4VSolid* createOctagon(double height, double basesside_dia, double pmtside_dia, double wall_thick){
    ...
    double outerA_r[2];
    outerA_r[0]=basesside_dia/2.;
    outerA_r[1]=pmtside_dia/2.+wall_thick;

    double outerB_r[2];
    outerB_r[0]=basesside_dia/sqrt(2);
    outerB_r[1]=pmtside_dia/2.+wall_thick;

    G4Polyhedra* outerA = new G4Polyhedra("outerA",0.25*M_PI,2*M_PI,4, 2, z_points, zero_array, outerA_r);
    G4Polyhedra* outerB = new G4Polyhedra("outerB",0*M_PI,2*M_PI,4, 2, z_points, zero_array, outerB_r);
    G4IntersectionSolid* outer_inter = new G4IntersectionSolid("theOctagon",outerA,outerB);

    double innerA_r[2];
    innerA_r[0]=basesside_dia/2.-wall_thick;
    innerA_r[1]=pmtside_dia/2.;

    double innerB_r[2];
    innerB_r[0]=basesside_dia/sqrt(2)-wall_thick;
    innerB_r[1]=pmtside_dia/2.;

    double z_inn[2]{0.1,-height-0.1};

    G4Polyhedra* innerA = new G4Polyhedra("innerA",0.25*M_PI,2*M_PI,4, 2, z_inn, zero_array, innerA_r);
    G4Polyhedra* innerB = new G4Polyhedra("innerB",0*M_PI,2*M_PI,4, 2, z_inn, zero_array, innerB_r);
...
G4VPhysicalVolume* QuLGDetectorConstruction::Construct(){
 ...
 //-------------------------------------------------------------
  ...
  //-------------------------------------------------------------
    ...
    double oil_hlength = pmtbox_height/2.-box_front_cap_thick; 
    ...
    G4ThreeVector dim = G4ThreeVector(14.47*cm, 14.47*cm, 117.6*cm);
  ...
  //                  0,                       //copy number
  //                  checkOverlaps); 
  G4Box *outer = new G4Box("volA", dim[0]/2.,dim[1]/2.,dim[2]/2.);
  G4Box *inner = new G4Box("volB", dim[0]/2.-sepThick,dim[1]/2.-sepThick,dim[2]/2.);  

  G4SubtractionSolid *sep_solid = new G4SubtractionSolid("sep_vol",outer,inner);

  //G4Box *sep_solid = new G4Box("sep_vol",0, 360*deg,6,2, scint_z,  m_sepInside, m_HexagoneROuter);
  G4LogicalVolume *sep_log = new G4LogicalVolume(sep_solid, Materials::FEP, "Sep_main_log");   
  G4PVPlacement *sep_phys = new G4PVPlacement(0,                       //no rotation
                    pos1,                    //at position
                    sep_log,             //its logical volume
  ...
  //-------------------------------------------------------------  
   ...
   G4Box *scint_solid = new G4Box("scint_vol",dim[0]/2.-sepThick,dim[1]/2.-sepThick,dim[2]/2.);
   ...
   //Extra reflectors 
   //-------------------------------------------------------------  
    G4Box* extra_reflector = new G4Box("extra_reflector", dim[0]/2.,dim[1]/2.,box_front_cap_thick/2.);
    G4Box* extra_reflector_inside = new G4Box("extra_reflector", dim[0]/2.-.1*mm,dim[1]/2.-.1*mm,box_front_cap_thick/2.);
   ...
   G4PVPlacement* extra_reflector_phys_1 = new G4PVPlacement(nullptr,G4ThreeVector(0,0,scint_height/2.+box_front_cap_thick/2.),extra_reflector_log,"extra_reflector_phys_1",scint_log,false,1,true);
   ...
   G4PVPlacement* extra_reflector_phys_2 = new G4PVPlacement(nullptr,G4ThreeVector(0,0,-scint_height/2.-box_front_cap_thick/2.),extra_reflector_log,"extra_reflector_phys_2",scint_log,false,2,true);
   ...
   G4Box* m_Hexagone_solid = new G4Box("pmt_housing", dim[0]/2.,dim[1]/2.,pmtbox_height/2.);
  ...
  for(unsigned int npm = 0; npm < 2; npm++){
   ...
   // Oil volume
   //---------------------------------------------------------------------------------------------------------------------- 
   G4Box* oil_box = new G4Box("min_oil", dim[0]/2. - box_side_wall_thick, dim[1]/2.- box_side_wall_thick, oil_hlength);
   ...
   G4PVPlacement* oil_phys = new G4PVPlacement(nullptr,{0,0,-box_front_cap_thick},oil_log,"oil_phys",m_Hexagone_log,false,npm,true);
...
//     //placed in a few more lines
     ...
     new G4PVPlacement(pmt_flip, G4ThreeVector(0,0,-oil_hlength+pmt_glass_z_from_front),pmt_glass_log, "PMT_glass_phys",oil_log,false,npm,true);    
   ...
   //----------------------------------------------------------------------------------------------------------------------
     ...
     G4PVPlacement* reflector_phys=new G4PVPlacement(pmt_flip,G4ThreeVector(0,0,-oil_hlength),reflector_log,"reflector_phys",oil_log,false,npm,true);
...
void QuLGDetectorConstruction::ConstructSDandField() {

  if (!fPmt_SD.Get()) {
     //Created here so it exists as pmts are being placed
     G4cout << "Construction /QuLGDet/pmtSD" << G4endl;
     //G4MultiFunctionalDetector* photocathode_MFD = new G4MultiFunctionalDetector("photocathodeMFD");     
     QuLGPMTSD* pmt_SD = new QuLGPMTSD("/QuLGDet/pmtSD");
